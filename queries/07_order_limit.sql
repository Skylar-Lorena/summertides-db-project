...
-- 1. Display attendees alphabetically (by last name, then first name).
...
SELECT ticket_id, attendee_id, purchase_date
FROM tickets
...
-- Bonus: the next three highest-rated vendors after the top three,
-- demonstrating OFFSET.
SELECT vendor_name, rating
FROM vendors
ORDER BY rating DESC
LIMIT 3 OFFSET 3